...
class CLIP(nn.Module):
    ...
    def __init__(
        ...
        test_text_tower: bool = False,  # Debugging only
    ):
        super().__init__()
        ...
        self._text_tower = text if test_text_tower else None

    def load_state_dict(
        self, state_dict: Mapping[str, Any], strict: bool = True, assign: bool = False
    ):
        if self._text_tower is None:
            return super().load_state_dict(
                state_dict=state_dict, strict=strict, assign=assign
            )
        else:  # Update: Load text tower using existing state dict if test_text_tower=True
            incompatible_keys = super().load_state_dict(
                state_dict=state_dict, strict=False, assign=assign
            )
            unexpected_keys = incompatible_keys.unexpected_keys
            if len(unexpected_keys) > 0:  # Shouldn't happen
                raise RuntimeError(
                    f"Found unexpected keys in CLIP state dict: {unexpected_keys}"
                )

            missing_keys = incompatible_keys.missing_keys
            text_tower_state_dict = {}
            for key in missing_keys:
                state_dict_key = key.replace("_text_tower.", "")
                text_tower_state_dict[state_dict_key] = state_dict[state_dict_key]

            self._text_tower.load_state_dict(
                text_tower_state_dict, strict=True, assign=assign
            )
    ...
    def encode_text(self, text, normalize: bool = False):
        ...
        if self._text_tower is not None:
            with torch.no_grad():
                x_text = self._text_tower(text)  # Debugging only
        ...
        if self._text_tower is not None:
            # This assert passes, so could easily just use TextTransformer.forward() then normalize
            assert torch.all(
                x == x_text
            ), "TextTransformer.forward() != CLIP.encode_text()"
